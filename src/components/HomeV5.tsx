...
import {
  ...
  type IconName,
  type Locale,
} from '@/data/didiLinksV5'
import { FeaturedVideoV5 } from '@/components/FeaturedVideoV5'
...
function PaintedCassetteV5({ className }: { className: string }) {
  return (
    <svg
      className={className}
      viewBox="0 0 214 122"
    ...
    >
      <path
        ...
        className="didi-v5-cassette-shell"
      />
      <path
        ...
        className="didi-v5-cassette-label"
      />
      <path
        ...
        className="didi-v5-cassette-window"
      />
      <path
        d="M24.4 30.9C22.1 39.3 21.2 82.4 24.5 91.1"
        className="didi-v5-cassette-sketch"
      />
      <path
        d="M190.2 30.2C193.5 42.1 193.2 79.5 189.3 91.4"
        className="didi-v5-cassette-sketch"
      />
      <circle cx="69" cy="65.5" r="13.2" className="didi-v5-cassette-reel" />
      <circle cx="145.4" cy="65.8" r="13.2" className="didi-v5-cassette-reel" />
      <path
        d="M82.7 66.8C101.9 72.1 119.4 72 132.4 66.2"
        className="didi-v5-cassette-tape"
      />
      <path
      ...
      <path
        ...
        className="didi-v5-cassette-sketch didi-v5-cassette-bottom"
      />
    </svg>
  )
}
...
export function HomeV5() {
  ...
  return (
    <main className="didi-v5-page">
      <div className="didi-v5-shell">
        <header className="didi-v5-top-bar">
          <LanguageSwitcher locale={locale} onLocaleChange={setLocale} />
        </header>

        <section className="didi-v5-profile-block" aria-labelledby="profile-title">
          <PaintedCassetteV5 className="didi-v5-profile-cassette" />
          <div className="didi-v5-profile-image-wrap">
            <img
              src="/didi-brand-art.png"
              alt={copy.profileAlt}
              className="didi-v5-profile-image"
            />
          </div>
          <div className="didi-v5-profile-copy">
            <p className="didi-v5-profile-kicker">
              <Sparkles size={16} aria-hidden="true" />
              {copy.eyebrow}
            </p>
            <h1 id="profile-title">{copy.brand}</h1>
            <p className="didi-v5-tagline">{copy.tagline}</p>
            <p className="didi-v5-intro">{copy.intro}</p>
          </div>
        </section>
        ...
        <section className="didi-v5-links-section" aria-labelledby="links-title">
          <div className="didi-v5-section-heading">
            <h2 id="links-title">{copy.linksTitle}</h2>
            <p>{copy.linksIntro}</p>
          </div>

          <div className="didi-v5-link-groups">
            {linkGroups.map((group) => (
              <section
                className="didi-v5-link-group"
                aria-labelledby={`${group.id}-title`}
                key={group.id}
              >
                <h3 id={`${group.id}-title`}>{group.title[locale]}</h3>
                <div className="didi-v5-link-list">
        ...
        <footer className="didi-v5-site-footer">
          <p>{copy.footer}</p>
...
function LanguageSwitcher({
  ...
  return (
    <nav className="didi-v5-language-switcher" aria-label="Language">
...
function StatusCard({ title, body }: { title: string; body: string }) {
  return (
    <section className="didi-v5-status-card" aria-labelledby="lesson-status-title">
      <div className="didi-v5-status-icon" aria-hidden="true">
        <Sparkles size={20} />
      </div>
      <div>
...
function CTAUpdateCard({
  ...
  return (
    <section className="didi-v5-update-card" aria-labelledby="updates-title">
      <div className="didi-v5-update-card-copy">
        <span className="didi-v5-mini-icon" aria-hidden="true">
          <Bell size={18} />
        </span>
        <div>
      ...
      <a
        className="didi-v5-primary-action"
        href={updateFormUrl}
        target="_blank"
...
function LinkCard({
  ...
  return (
    <a
      ...
      className={`didi-v5-link-card${isComingSoon ? ' is-coming-soon' : ''}`}
      href={link.href}
      onClick={handleClick}
      rel={isComingSoon ? undefined : 'noopener noreferrer'}
      target={isComingSoon ? undefined : '_blank'}
    >
      <span className="didi-v5-link-icon" aria-hidden="true">
        <Icon size={22} />
      </span>
      <span className="didi-v5-link-text">
        <strong>{link.label[locale]}</strong>
        <small>{link.description[locale]}</small>
      </span>
      {isComingSoon ? (
        <span className="didi-v5-link-badge">{copy.comingSoon}</span>
      ) : (
        <ArrowUpRight className="didi-v5-link-arrow" size={18} aria-hidden="true" />
      )}
    </a>
  )
}
